...
// accessAcDbObjectForRead:  For Internal Use Only
Acad::ErrorStatus __stdcall
...
public:
                    ...
                    bool bOpenErased)
    {
        ...
        if (Acad::eNotOpenForRead == mEs) {
            mEs = accessAcDbObjectForRead((AcDbObject*&) pObj,
                                          id,
                                          ACDB_CLASS::desc(),
                                          mbWasOpened,
                                          bOpenErased);
...
public:
    WritableAcDbObject()
        : mpObj(NULL), mEs(Acad::eNullObjectPointer)
    {
    }

    WritableAcDbObject(AcDbObject* pObj, bool enableNow = true)
        : mpObj(NULL), mEs(Acad::eNullObjectPointer)
    {
...
private:
    ...
    AcDbSmartObjectPointer(AcDbSmartObjectPointer & pObject);
    AcDbSmartObjectPointer& operator=(AcDbSmartObjectPointer & pObject);
...
AcDbSmartObjectPointer<ACDB_CLASS>::AcDbSmartObjectPointer(
    ...
    AcDb::OpenMode mode,
    bool           openErased)
    : mReadable(mReadable.mpObj, objId, openErased),
      mWritable(mReadable.object(), AcDb::kForWrite == mode)
{
