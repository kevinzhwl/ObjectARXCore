//
//
//  Copyright 2004-2008 by Autodesk, Inc.
//
...
typedef std::vector<AcDMMXMLAttribute> AcDMMXMLAttributeVec;
...
public:
    ...
    void AddXMLAttribute(const wchar_t *ns, const wchar_t *nsUrl, const wchar_t *attName, const wchar_t *attValue)
    {
        if (NULL != ns && NULL != nsUrl && NULL != attName && NULL != attValue)
            m_XMLAttributes.push_back(AcDMMXMLAttribute(ns,nsUrl,attName,attValue));
    }
