...
public:
    ...
    //
    int                numResults (AcGeIntersectError& err) const;
                    // Counts the number of intersection results of any dimension.
    AcGeCurve3d*   intCurve (int intNum, Boolean isExternal, AcGeIntersectError& err) const; 
					// Returns NULL if the dimension of  this intersection is not 1.
    AcGeCurve2d*   intParamCurve(int num, Boolean isExternal, Boolean isFirst, AcGeIntersectError& err) const;
					// Returns NULL if the dimension of  this intersection is not 1.
    ...
    void               getIntPointParams (int intNum,
                                         AcGePoint2d& param1, AcGePoint2d& param2, AcGeIntersectError& err) const;
	void getIntConfigs (int intNum, AcGe::ssiConfig& surf1Left,  AcGe::ssiConfig& surf1Right, 
							...
							AcGe::ssiType& intType, int& dim, AcGeIntersectError& err ) const; 
    int		getDimension (int intNum, AcGeIntersectError& err) const;
	AcGe::ssiType	getType(int intNum, AcGeIntersectError& err ) const;
