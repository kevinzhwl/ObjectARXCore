...
{
public:
    AcDbXrefGraphNode(const char* pName = NULL,
    ...
    void		setBtrId	(AcDbObjectId);
    void		setDatabase	(AcDbDatabase*);

    Adesk::Boolean	isNested	() const;
...
public:
    ...
    AcDbXrefGraphNode*	xrefNode	(const char* pName) const;
...
//
extern Acad::ErrorStatus acedGetCurDwgXrefGraph(AcDbXrefGraph&,
                                            Adesk::Boolean includeGhosts =
					        Adesk::kFalse);
...
inline const char* AcDbXrefGraphNode::name() const { return mpName; }
...
inline void AcDbXrefGraphNode::setBtrId(AcDbObjectId id) { mBtrId = id; }
inline void AcDbXrefGraphNode::setDatabase(AcDbDatabase* pDb) { setData(pDb); }

inline Adesk::Boolean AcDbXrefGraphNode::isNested() const
