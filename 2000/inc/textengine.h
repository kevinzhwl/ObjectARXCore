...
typedef void (*UnicodeCallback)     (long, unsigned short const *, int, void *);
...
public:
	...
	// This routine converts the mutlibyte string into a unicode string.
	// It also processes any control codes that are embedded in the multibyte
	// string. The converted unicode is passed back through a unicode callback
	// function. In addition, any overscores/underscores that might result
	// from the processing of control codes will be drawn through the polyline
	// callback. The vectors representing the overlines and underlines will be
	// in unit coordinates as well. Currently, this is implemented for truetype
    // text only. Needs to be updated for shape text as well.
    virtual void tessellate (AcGiTextStyle &    ts,
                             char const *       pMulitbyteString,
