...
class AcDbDictionaryIterator: public AcRxObject
//
...
public:
    ...
    //
    bool              has       (const char*  entryName) const;
    bool              has       (AcDbObjectId objId) const;
    Adesk::UInt32     numEntries() const;

    // Remove entries.
    //
    Acad::ErrorStatus remove(const char *  key);
    Acad::ErrorStatus remove(const char *  key,
    ...
    // Reset an entry.
    //
    bool              setName(const char*   oldName,
                              const char*   newName);
    Acad::ErrorStatus setAt  (const char*   srchKey,
                              AcDbObject*   newValue,
    ...
    //
    AcDbDictionaryIterator*
        newIterator(AcRx::DictIterType iterType = AcRx::kDictSorted);
