...
#define AC_ACARRAY_H
//
// (C) Copyright 1993,1995-1999 by Autodesk, Inc.
//
...
public:
    ...
    bool                operator == (const AcArray<T,R>&);
...
#define lowMemBail() assert(false)
...
AcArray<T,R>::operator == (const AcArray<T,R>& cpr)
{
...
AcArray<T,R>::setLogicalLength(int n)
{
    assert(n >= 0);
    if (n > mPhysicalLen) {
        int minSize = mPhysicalLen + mGrowLen;
        if ( n > minSize)
            minSize = n;
