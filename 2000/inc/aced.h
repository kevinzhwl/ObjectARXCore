...
extern int acedCmdLookup(const char* cmdStr, int globalLookup,
                         struct AcEdCommandStruc* retStruc,
                         int skipUndef = FALSE);

extern int acedCmdLookup2(const char* cmdStr, int globalLookup,
                         struct AcEdCommandStruc* retStruc,
...
extern Acad::ErrorStatus acedSetCurrentVPort(int vpnumber);
extern Acad::ErrorStatus acedSetCurrentVPort(AcDbViewport* pVp);
...
extern Acad::ErrorStatus acedSetCurrentUCS(AcGeMatrix3d mat);
extern Acad::ErrorStatus acedGetCurrentUCS(AcGeMatrix3d& mat);
extern Acad::ErrorStatus acedRestorePreviousUCS();
...
//
extern void acedEditToleranceInteractive(AcDbFcf* pTol);

// Replaces the AcDbMText::edit() method
// pMText must be open at least for Read, preferably for Write
extern int acedEditMTextInteractive(AcDbMText* pMText);

//  AutoCAD color to RGB conversion function
//
extern unsigned long     acedGetRGB(int color);

extern Acad::ErrorStatus acedGetCurrentSelectionSet(AcDbObjectIdArray& sset);
