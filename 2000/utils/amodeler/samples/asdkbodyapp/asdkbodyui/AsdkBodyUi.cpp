...
static AsdkBody* getNewFace(char* kw);
...
static double getDistance(AcGePoint3d fromPt, char *prompt = "", double defaultDist = 0.0);
static int getInt(char *prompt = "", int defaultValue = 0);
static double getReal(char *prompt = "", double defaultValue = 0.0);
...
CommandInfo commandInfo[] =
{
    ...
    "AENDPOINTREVOLVE",     &createEndpointRevolution,  "Sweep around the endpoint",
    "ASKIN",                &createSkin,                "Create solid out of multiple profiles",
...
const char* gstrCmdGrpName = "ASDKBODYUI_COMMANDS";
...
acrxEntryPoint( AcRx::AppMsgCode msg, void *pkt )
{
    ...
    case AcRx::kInitAppMsg:
        ...
        acrxLoadModule("AsdkBodyObj.dbx",1);
    ...
    case AcRx::kUnloadAppMsg:
		...
		acrxUnloadModule("AsdkBodyObj.dbx");
        break;
    }
...
createBox()
{
    ...
    try
    {
        AcGePoint3d corner1 = getPoint("\nCorner of box: ");
        AcGePoint3d corner2 = getPoint("\nOther corner: ");
        ads_printf( "\n" );
        ...
        if (vec[X] == 0.0)
            vec[X] = getDistance(corner2, "\nWidth: ");
        if (vec[Y] == 0.0)
            vec[Y] = getDistance(corner2, "\nDepth: ");
        if (vec[Z] == 0.0)
            vec[Z] = getDistance(corner2, "\nHeight: ");
...
createCylinder()
{
    ...
    try
    {
        ...
        double radius = getDistance(axisStart, "\nRadius: ");
        int approx = getInt("\nEnter # of lines to approximate a circle <32>:", 32);
...
createCone()
{
    ...
    try
    {
        ...
        double radius1 = getDistance(axisStart, "\nStart radius: ");
        double radius2 = getDistance(axisEnd, "\nEnd radius: ");
        int approx = getInt("\nEnter # of lines to approximate a circle <32>:", 32);
...
createPipe()
{
    ...
    try
    {
        ...
        double outerRadius = getDistance(axisStart, "\nOuter radius: ");
        double innerRadius = getDistance(axisStart, "\nInner radius: ");
        int approx = getInt("\nEnter # of lines to approximate a circle <32>:", 32);
...
createPipeConic()
{
    ...
    try
    {
        ...
        double outerRadius1 = getDistance(axisStart, "\nFirst outer radius: ");
        double innerRadius1 = getDistance(axisStart, "\nFirst inner radius: ");
        double outerRadius2 = getDistance(axisEnd, "\nSecond outer radius: ");
        double innerRadius2 = getDistance(axisEnd, "\nSecond inner radius: ");
        int approx = getInt("\nEnter # of lines to approximate a circle <32>:", 32);
...
createTetrahedron()
{
    ...
    try
    {
        AcGePoint3d corner1 = getPoint("\nCorner one: ");
        AcGePoint3d corner2 = getPoint("\nCorner two: ");
        AcGePoint3d corner3 = getPoint("\nCorner three: ");
        AcGePoint3d corner4 = getPoint("\nCorner four: ");
        ads_printf( "\n" );
...
createTorus()
{
    ...
    try
    {
        ...
        double majorRadius = getDistance(axisStart, "\nMajor radius: ");
        double minorRadius = getDistance(axisStart, "\nMinor radius: ");
        int majorApprox = getInt("\nEnter # of lines to approximate major radius <32>:", 32);
        int minorApprox = getInt("\nEnter # of lines to approximate minor radius <32>:", 32);
...
createReducingElbow()
{
    ...
    try
    {
        AcGePoint3d elbowCenter = getPoint("\nElbow center: ");
        AcGePoint3d endCenter1 = getPoint("\nEnd one center: ");
        double endRadius1 = getDistance(endCenter1, "\nEnd one radius: ");
        AcGePoint3d endCenter2 = getPoint("\nEnd two center: ");
        double endRadius2 = getDistance(endCenter2, "\nEnd two radius: ");
        int majorApprox = getInt("\nEnter # of lines to approximate major radius <32>:", 32);
        int minorApprox = getInt("\nEnter # of lines to approximate minor radius <32>:", 32);
...
createRectToCircleReducer()
{
    ...
    try
    {
        AcGePoint3d baseCorner = getPoint("\nBase corner: ");
        AcGeVector3d baseSizes3d = getPoint("\nBase sizes: ") - baseCorner;
        AcGeVector2d baseSizes(baseSizes3d.x, baseSizes3d.y);
        AcGePoint3d circleCenter = getPoint("\nCircle center: ");
        double circleRadius = getDistance(circleCenter, "\nCircle radius: ");
...
createExtrusion()
{
    ...
    try 
    {
        ...
        {
            try {
                fixedPt = getPoint("Select fixed point for scale/twist <0,0,0>: ");
            }
            catch (int caught_adsrc)
            {
                if (caught_adsrc)
...
createEndpointRevolution()
{
    ...
    try 
    {
		...
		pBody->createEndpointRevolution(vertices, vertexData, iVertices, 
            normal, (double)revolutionAngle, approx);
	}
    catch (int caught_adsrc)
    //
    // User probably cancelled.  Clean up and bail out.
...
// createSkin()
//
// Prompts between profiles for morphing maps.
//
void
createSkin()
{
    const int iMaxProfiles = 64;
    AsdkBody *pBody = NULL;
	AsdkBody *profiles[iMaxProfiles];
	memset(profiles,sizeof(AsdkBody*)*iMaxProfiles,0);
    bool bIsClosed;
	MorphingMap *morphMaps[iMaxProfiles];
	memset(morphMaps,sizeof(MorphingMap*)*iMaxProfiles,0);
    int i, iPrev, iLast, iProfiles;
	
    try
    {
        ads_printf("\nSelect profiles for skin\n");
        for (i = 0, iPrev = i - 1; i < iMaxProfiles; i++, iPrev++)
        {
            iProfiles = i;
            AsdkBody* pProfile = getFace();
            ads_printf("\n");
            ...
            if (iPrev < 0)
                continue;  // no morphing map for first profile

            morphMaps[iPrev] = queryMorphingMap(profiles[iPrev], profiles[i]);
        }
        iLast = iProfiles - 1;

	    ads_printf("\nClose body? ");
	    bIsClosed = getYesNo(FALSE);

        if (bIsClosed)
            morphMaps[iLast] = queryMorphingMap(profiles[iLast], profiles[0]);
...
aIntersect()
{
    ...
    if (NULL != pFirst) 
    {
        ...
        {
            ads_printf( "Null AsdkBody created - deleted\n" );
            pFirst->erase();
        }
        pFirst->close();
        ads_entupd(first_ent);
...
getFace()
{
    ...
    ads_initget(0, "Points polyLine");
	adsrc = ads_getkword("Points/<polyLine>: ", kw);
    if (adsrc <= RTERROR) 
		throw adsrc;
...
getMorphingMap(const Body& startProfile, const Body& endProfile)
{
	...
	if (endCount > startCount)
	{
		...
		for(int i = 0; i < endCount; i++) {
			...
			ads_printf("Map vertex %d to vertex [0-%d] <%d> :", 
					i, startCount - 1, defaultVertex);
            vertex = getInt("", defaultVertex);
...
queryMorphingMap(AsdkBody* pBody1, AsdkBody* pBody2)
{
	...
	if (bGetMorphingMap)
        return new MorphingMap(getMorphingMap(pBody1->body(), pBody2->body()));
    else
        return NULL;
}
...
// to a AcDb3dPolyline.  Returns NULL if this doesn't work.
//
...
static double
getDistance(AcGePoint3d fromPt, char *prompt, double defaultDist)
{
    int adsrc;
    ads_point p;
...
static int
getInt(char *prompt, int defaultValue)
{
    int i;
...
static double
getReal(char *prompt, double defaultValue)
{
    double dbl;
