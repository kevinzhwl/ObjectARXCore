...
enum ErrorCode
{
    ...
    eProfilePolygomNustContainAtLeastOnePoint,
...
#define checkError(testExpr, err) checkErrorFunc(testExpr, err, __FILE__, __LINE__)

inline void checkErrorFunc(bool testExpr, ErrorCode err, const char* file, int line)
{
    if (!testExpr)
    {
        printf("checkError(ErrorCode=%d, File=%s, Line=%d)\n", (int)err, file, line);
        throw err;
    }
}
#else
...
// ErrorCode
//
DllImpExp char* errorString(ErrorCode);
