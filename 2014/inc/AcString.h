...
class AcString : public AcHeapOperators
{
  public:
    ...
    ACBASE_PORT AcString(const char *psz);
    ...
    ACBASE_PORT const    char * kszPtr() const;
    ...
    ACBASE_PORT int find(ACHAR ch) const;
    ...
    // ch : input char to search for
    ACBASE_PORT int findRev(ACHAR ch) const;
    ...
    // nNumChars : input number of chars (not bytes) to retrieve.
    //             if nNumChars is -1, then return the rest of the string
    ACBASE_PORT AcString substr(int numChars) const;
    ...
    ACBASE_PORT AcString & assign(char ch);

    // assign a Unicode char to the string
    // wch : input char to assign
    ACBASE_PORT AcString & assign(wchar_t wch);
   ...
   ACBASE_PORT  AcString & assign(const char *psz);
    ...
    // append an ansi char to the end of the string
    // ch : input char to append
    ACBASE_PORT AcString & append(char ch);

    // append a Unicode char to the end of the string
    // wch : input char to append
    ACBASE_PORT AcString & append(wchar_t wch);
    ...
    // Copy the string and append a char to it
    // ch : input char to append to the string copy
    ACBASE_PORT AcString concat(char ch) const;

    // Copy the string and append a Unicode char to it
    // ch : input char to append to the string copy
    ACBASE_PORT AcString concat(wchar_t wch) const;
    ...
    ACBASE_PORT AcString precat(ACHAR ch) const;
    ...
    ACBASE_PORT int  compare(wchar_t wch) const;
    ...
    // wch : input char to compare to
    ACBASE_PORT int  compareNoCase(wchar_t wch) const;
    ...
    ACBASE_PORT bool operator <= (const char ch) const;

    // Compare the string for less than or equal to a Unicode char
    // wch : input char to compare to
    ACBASE_PORT bool operator <= (const wchar_t wch) const;
...
#if  defined(_AFX) || defined (_ADESK_MAC_)
    //
