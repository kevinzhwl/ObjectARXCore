...
public:
    ...
    AcDbGroup();
    AcDbGroup(const char* grpDesc, bool selectable = true);
    virtual ~AcDbGroup();

    AcDbGroupIterator*  newIterator();

    const char*         description() const;
    Acad::ErrorStatus   setDescription(const char* grpDesc);

    bool                isSelectable() const;
    Acad::ErrorStatus   setSelectable(bool selectable);

    const char*         name() const;    //deprecated. New code should use getName.
    Acad::ErrorStatus   getName(char*& name) const;
    Acad::ErrorStatus   setName(const char* newName);
