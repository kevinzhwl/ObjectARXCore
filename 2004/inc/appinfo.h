...
public:
    ...
    const char *appDesc() const;
    void setAppDesc(const char *);

    bool appImplements(const char *) const;
    ...
    AcadApp::ErrorStatus writeGroupNameToRegistry(const char* group);

    AcadApp::ErrorStatus writeCommandNameToRegistry(const char* globalCmdName,
                                                    const char* localCmdName);
