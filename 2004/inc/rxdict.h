...
public:
    ...
    virtual AcRxObject*    at      (const char* key) const = 0;
    virtual AcRxObject*    at      (Adesk::UInt32 id) const = 0;

    AcRxObject*            atPut   (const char* key, AcRxObject* pObject);
    virtual AcRxObject*    atPut   (const char* key, AcRxObject* pObject,
        Adesk::UInt32& retId) = 0;
    virtual AcRxObject*    atPut   (Adesk::UInt32 id, AcRxObject* pObject) = 0;

    virtual Adesk::Boolean resetKey(const char* oldKey, const char* newKey) = 0;
    virtual Adesk::Boolean resetKey(Adesk::UInt32 id, const char* newKey) = 0;

    virtual Adesk::Boolean atKeyAndIdPut(const char* newKey, Adesk::UInt32 id,
        AcRxObject* pObject) = 0;

    virtual AcRxObject*    remove  (const char* key) = 0;
    virtual AcRxObject*    remove  (Adesk::UInt32 id) = 0;

    virtual Adesk::Boolean has     (const char* entryName) const = 0;
    virtual Adesk::Boolean has     (Adesk::UInt32 id) const = 0;

    virtual Adesk::UInt32  idAt    (const char* key) const = 0;

    virtual const char*    keyAt   (Adesk::UInt32 id) const = 0;
...
inline AcRxObject* AcRxDictionary::atPut(const char* key, AcRxObject* pObj)
