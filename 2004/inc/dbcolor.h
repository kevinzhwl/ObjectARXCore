//
// (C) Copyright 1993-1999,2002 by Autodesk, Inc.
//
...
public:
    ...
    virtual Acad::ErrorStatus   setNames(const char *colorName,
                                         const char *bookName = NULL) = 0;
    virtual const char *        colorName(void) const = 0;
    virtual const char *        bookName(void) const = 0;
    virtual const char *        colorNameForDisplay(void) = 0;
...
typedef std::vector<AcCmColorBase*> AcCmColorBaseArray;
...
public:
    ...
    AcCmTransparency() { mAM.method = kByLayer; }
