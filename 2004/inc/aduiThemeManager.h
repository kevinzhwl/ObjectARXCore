...
public:
    ...
    CAdUiTheme * GetTheme(const char *pThemeName);
    BOOL ReleaseTheme(CAdUiTheme * pTheme);

    BOOL HasTheme(const char *pThemeName) const;

    void SystemColorChanged();

private:
    int  RemoveTheme(const char *pThemeName);
    void Cleanup();
    int  GetThemeIndex(const char *pThemeName) const;
