...
createEntity(AcDbEntity*& pEntity)
{
    ...
    char opt[128];
   	while (Adesk::kTrue) {
		...
		acedInitGet(NULL, "Box Wedge Frustum Sphere Torus");
		if (acedGetKword("Box/Wedge/Frustum/Sphere/<Torus>: ", opt) == RTCAN) {
			entType = 0;
			break;
		}

        // Map the user input to a valid solid primitive type
		if ((strcmp(opt, "Torus") == 0) || (strcmp(opt, "") == 0)) {
			entType = 1;
			break;
        } else if (strcmp(opt, "Sphere") == 0) {
			entType = 2;
			break;
        } else if (strcmp(opt, "Frustum") == 0) {
			entType = 3;
			break;
        } else if (strcmp(opt, "Wedge") == 0) {
			entType = 4;
			break;
        } else if (strcmp(opt, "Box") == 0) {
			entType = 5;
			break;
		}
    }
