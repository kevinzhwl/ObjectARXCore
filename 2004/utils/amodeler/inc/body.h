...
public:
    ...
    Body boolOper(Body&, BoolOperType, bool hideZeroSlopeEdges, TopologyChangeCallback*);
    Body boolOper(Body&, BoolOperType); // TODO Jiri Kripac: Left For DLL compatibility
    ...
    //
    void section(const Plane&, const Point3d& point);
    ...
    static Body skin          (Body*              profiles[],
                               ...
                               bool               markInnerProfileEdgesAsApprox,
                               bool               ckeckInputProfiles);

    // TODO Jiri Kripac: For DLL compatibility
    //
    static Body skin          (Body*              profiles[],
                               int                numProfiles,
                               bool               isClosed,
    ...
    void hideDisplay   (const Transf3d&            projectionTransf, 
                        ...
                        HideIntegerTransform*); // Output argument, may be NULL

    // TODO Jiri Kripac: Left for the DLL compatibility. Should be removed
    // and optional arguments should be added to the method above
    //
    void hideDisplay   (const Transf3d&            projectionTransf, 
                        OutputPolylineCallback*,
    ...
    static Body restore(const char* fileName);  // Returns a newly created body
    ...
    void        saveToSat(const char* fileName, bool colorsAlso = false) const;
    ...
    //
    void stitchFaces(bool splitEdges, bool orientFaces, bool addMissingFaces);

    // TODO Jiri Kripac: Left for DLL compatibility, equivalent to stitchFaces(false, false, false)
    //
    void stitchFaces(); 
    ...
    //
    void mergeCoplanarEntities(bool bridgeEdgesOnly,
                               bool sameColorOnly,
    ...
    // TODO Jiri Kripac: Left for DLL compatibility, equivalent to
    // mergeCoplanarEntities(true, true). Should be removed and arguments in
    // the method above should be made optional
    //
    ...
    // Extracts the shell that contains the given face from the body and returns
    // it in a new body by moving the topological entities from this body to the 
    // new body. Only topological entities that cannot be moved because they are 
    // also needed in this body are copied and not moved.
    //
    ...
    //
    bool     isValid(int validityCheckingLevel) const; 
    bool     isValid() const; // TODO Jiri Kripac: Left for DLL compatibility
...
// TODO Jiri Kripac: All AModeler method and function names start with lowercase 
// but Spatial added this function starting with uppercase. For DLL 
// compatibility reasons, I left the old name
//
