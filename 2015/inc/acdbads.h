...
ACDB_PORT int  acdbXdRoom (const ads_name ent, Adesk::Int32 *result);
ACDB_PORT int  acdbXdSize (const resbuf *rb, Adesk::Int32 *result);

// Function to save a string ensuring no buffer overflow if,
// when converted, characters expand to \U+ sequences 
ACDB_PORT ACHAR * acdbXStrSave (ACHAR *pSource, ACHAR **pDest);

// Function to uppercase a table name
ACDB_PORT int     acdbXStrCase (ACHAR *str, size_t strLen);
