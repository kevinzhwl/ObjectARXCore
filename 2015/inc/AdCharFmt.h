...
class AdCharFormatter
{
  public:
    ...
    {
        AdCharFmt_Assert(bToAnsi || !bUseCIF);  // No CIF with utf-8
        AdCharFmt_Assert(nDestBufSize >= 2);
        if (nDestBufSize < 2)
            return 0;
        AdCharFmt_Assert(!bUseCIF || nDestBufSize >= 7);  // CIF needs 7
        AdCharFmt_Assert(bToAnsi || nDestBufSize >= 4);  // utf8 needs 4
        if (wch >= 0 && wch <= 0x7f) {  // simple ascii
            ...
            pDestBuf[0] = (char)wch;
            AdCharFmt_Assert(pDestBuf[0] == wch);
            return 1;
        }
...
#ifdef _ADESK_MAC_
        // Well, the comment above just isn't true on the MAC, see DID 1326233.
        // Our extended chars in that bug get returned with nCvted == 0.
        // I couldn't find a proper fix down in WideCharToMultiByteImp()
        // so I'm handling it here for now, Mac only.
        if (nCvted == 0 && bToAnsi && bUseCIF) {
            putCIF<char>(wch, pDestBuf);
            return 7;   // length of CIF string
        }        
#endif        
        AdCharFmt_Assert(nCvted >= 1);
        AdCharFmt_Assert(nCvted <= 4);
        AdCharFmt_Assert(nCvted <= 2 || !bToAnsi);
        AdCharFmt_Assert(!bUsedDefaultChar || bToAnsi);
                        ...
                        char *pChNative = NULL) // Equivalent ansi char(s).
    {
        if (wch >= 0 && wch <= 0x7f)
        {
            if (pChNative != NULL) {
                *pChNative = (char)wch;
                AdCharFmt_Assert(*pChNative == (char)wch);
            }
            return 1;
        }
        ...
        bool b2byteCharToCIF = false) //sometimes it is required to Convert non-single byte characters to CIF, no matter it is native or not.
    {
        ...
        while(*lpReadPtr)
        {
            ...
            int ret =  isNativeToCodePage(wch, CP_ACP);
            if (!(b2byteCharToCIF && ret == 2) && ret > 0)
            {
                if (szDst != NULL)
                    *szDst++ = wch;
            ...
            else
            {
                if (szDst != NULL)
                {
                    AdCharFmt_Assert(cchDstSize - cchWritten >= 8);
...
#else
        STUB_WARNING(wcharNonNativeToCIF AdCharFmt.h);
        return 0;
#endif
    }
    ...
    static int wcharNonNativeToRTF(
        unsigned nCodePage,     // Conversion code page.
        const wchar_t *szSrc,   // Source wide string.
