...
public:
    ...
    virtual Acad::ErrorStatus logTerminalError(const char *pErrorString) = 0;

    // log an Abort, Retry, Ignore error
    virtual Acad::ErrorStatus logARIError(const char *pErrorString) = 0;

    // log a severe error
    virtual Acad::ErrorStatus logSevereError(const char *pErrorString) = 0;

    // log an error
    virtual Acad::ErrorStatus logError(const char *pErrorString) = 0;

    // log a warning
    virtual Acad::ErrorStatus logWarning(const char *pWarningString) = 0;


    // log a message
    virtual Acad::ErrorStatus logMessage(const char *pMessageString) = 0;

    // log an informational message
    virtual Acad::ErrorStatus logInformation(const char *pMessageString) = 0;
