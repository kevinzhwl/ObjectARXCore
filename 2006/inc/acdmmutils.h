...
inline void AcDMMWideString::MoveChars(int iStartOffset, PCWIDESTR pwsz,
    size_t iChars)
{
    assert(iStartOffset >=0);
    assert(iStartOffset <= static_cast <int> (m_iLength));
...
{
    Empty();
    m_iLength = wcslen(pwsz);
    Alloc(pwsz,m_iLength);

    return(*this);
}

inline size_t AcDMMWideString::GetLength() const
{
    return(m_iLength);
}
...
typedef std::vector<AcDMMWideString> AcDMMStringVec;
...
typedef std::vector<AcDMMResourceInfo> AcDMMResourceVec;
