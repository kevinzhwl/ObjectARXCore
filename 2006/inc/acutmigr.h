...
//
// (C) Copyright 1998-1999,2004-2005 by Autodesk, Inc. 
//
...
newBuffer(ACHAR *&pOutput, size_t size)
{ return acutNewBuffer(pOutput, size); }

inline Acad::ErrorStatus
newString(const ACHAR* pInput, ACHAR*& pOutput)
{ return acutNewString(pInput, pOutput); }

inline Acad::ErrorStatus
updString(const ACHAR* pInput, ACHAR*& pOutput)
{ return acutUpdString(pInput, pOutput); }

inline void
delString(ACHAR*& pString)
{ acutDelString(pString); }

inline void
delBuffer(ACHAR*& pString)
{ acutDelString(pString); }
