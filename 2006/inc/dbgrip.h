...
typedef AcArray<AcDbGripData> AcDbGripDataArray;
...
typedef AcDbGripOperations::ReturnValue (*GripRtClkHandler)(AcDbGripDataArray& hotGrips,
    const AcDbObjectIdArray& ents, char*& menuName, HMENU& menu,
...
typedef const char* (*GripToolTipPtr)(AcDbGripData* pThis);
...
inline AcDbGripData::AcDbGripData() : m_pAppData(NULL), m_pHotGripFunc(NULL),
    m_pHoverFunc(NULL), m_pRtClk(NULL), m_pWorldDraw(NULL),
    m_pViewportDraw(NULL), m_pGripOpStatFunc(NULL), m_pToolTipFunc(NULL),
    m_pHoverDimensionFunc(NULL), m_pHotGripDimensionFunc(NULL),
    m_bitFlags(0), m_pAltBasePt(NULL),
...
inline AcDbGripData::AcDbGripData(const AcGePoint3d& pt, void* AppData,
    ...
    m_gripPt(pt), m_pAppData(AppData), m_pHotGripFunc(hotGrip),
