...
Add the template arguments for the AcadToolImpl class:
	...
	Note:  Because they are template LPCTSTR arguments, the Tools Name and image name must be declared globally
	(but not statically) as TCHARs.  The best place to do this is in the Tool's .cpp file, declaring them as extern in
...
AcadToolImpl.
	...
	extern TCHAR szEmployeeName[MAX_PATH]; // Declared in the tools .cpp file
	extern TCHAR szEmployeeImage[MAX_PATH];// -   -
...
A typical usage of these functions might be:
		...
		AcTcCatalog *pCatalog=tool.CreateStockToolATC(_T("EmployeeCatalog"));
		AcTcPalette *pPalette=tool.CreatePaletteATC(pCatalog,_T("EmployeePalette"));
...
template <class TT,class T,const CLSID* pclsid,LPCTSTR szToolName,LPCTSTR szToolImage,const IID* piid = &__uuidof(T),const GUID* plibid = &CAtlModule::m_libid> 
class ATL_NO_VTABLE AcadToolImpl : 
	...
	public IAcadToolDropTarget,
	public IPropertyUnspecified,
	public IPerPropertyDisplayImpl<AcadToolImpl>,
...
public:
	...
	AcCmColor m_Color;
	TCHAR m_szLayer[512]; // Represents a Layer combo box in the tool property inspector 
...
public:
	...
	STDMETHOD(Load) (/*[in]*/ IUnknown* pXmlElement)
	{
		...
		if(GetChildNode(pEntityNode, "Tool",&pAttributeNode))
	...
	STDMETHOD(Save) (/*[in]*/ IUnknown* pXmlElement)
	{
		...
		if(!GetChildNode(pParentNode,"Tool",&pEntityNode))
			AddChildNode(pParentNode, "Tool", MSXML::NODE_ELEMENT, &pEntityNode);
	...
	STDMETHOD(Execute) (/*[in]*/ long lFlag, /*[in]*/ DWORD /*HWND*/ hWnd, /*[in]*/ DWORD point, /*[in]*/ DWORD dwKeyState)
	{
		...
		_tcscpy(m_FlyoutTools,_T(""));//Reset this each time the tool is run.
	...
	STDMETHOD(GetData) (/*[in]*/ VARIANT *pObjUnkArray, /*[in]*/ VARIANT *pDynUnkArray)
	{
		...
		return GetUnknownArrays(1, &pUnk, pObjUnkArray, pDynUnkArray);
	}
	...
	{
		// return our IAcadToolDropTarget interface...
		return _InternalQueryInterface(IID_IAcadToolDropTarget, (void**) ppDropTarget);
	}
    ...
    STDMETHOD(SetToolProperties) (/*[in]*/ IUnknown* pProp)
	{
		...
		m_pToolProp=NULL;// Avoid the ATL Assert.
		HRESULT hr = pProp->QueryInterface(IID_IAcadToolProperties, (void**) &m_pToolProp);
				
		CComQIPtr<IAcadToolProperties2> pToolProp2(m_pToolProp);
		if(!pToolProp2)
			return E_FAIL;

		VARIANT_BOOL vBool;
        pToolProp2->get_IsFlyoutEnabled(&vBool);
		if(vBool==VARIANT_TRUE)
		{
			hr=pToolProp2->get_ShapePackageId(&m_FlyoutPackageId);
			hr = pToolProp2->get_CurrentShapeId(&m_CurrentShapeId);
		}
		return S_OK;
	}
	...
	STDMETHOD(GetContextMenu) (/*[in]*/ int nContextFlag, /*[in]*/ DWORD /*HMENU*/ hMenu, /*[in]*/ UINT idCmdFirst,/*[in]*/  UINT idCmdLast, /*[out, retval]*/ DWORD* pFlag){return E_NOTIMPL;}
	STDMETHOD(GetCommandString) (/*[in]*/ UINT idCmd, /*[out, retval]*/BSTR* pbstrHelpText){return E_NOTIMPL;}
    STDMETHOD(InvokeCommand) (/*[in]*/ UINT idCmd, /*[in]*/ DWORD /*HWND*/ hWnd){return E_NOTIMPL;}
	...
	//************************************************************************************
	//IAcadTool2 members -override these if you support multiple editing

	 STDMETHOD(BeginMultipleEdit)(/*[in]*/ VARIANT tools,  /*[in]*/ VARIANT	stockToolIds)
	{
		return S_OK;
	}
	STDMETHOD(EndMultipleEdit)(/*[in]*/ VARIANT tools,  /*[in]*/ VARIANT	stockToolIds,/*[in]*/ VARIANT_BOOL bEditCancelled)
	{
		return S_OK;
	}
	STDMETHOD(MultipleEdit)(/*[in]*/ VARIANT tools,  /*[in]*/ VARIANT stockToolIds, /*[in]*/ LONG /*HWND*/ hWndParent, /*[out, retval]*/ VARIANT_BOOL* pRet)
	{
		m_hWndEditWindow=(HWND)hWndParent;	
		return S_OK;
	}
	...
	STDMETHOD(Drop) (IDataObject * pDataObj,DWORD grfKeyState,POINTL pt,DWORD * pdwEffect,GUID * pCurrentStockToolId,GUID * pParentId,VARIANT * pNewToolIds)
	{
		*pdwEffect = DROPEFFECT_COPY;
		...
		if(!pMgr)
			return E_FAIL;
		AcTcPalette *pParent=(AcTcPalette*)pMgr->FindItem(*pParentId);
		...
		rgsaBound.cElements = 256; // Assumes - Maximum 256 elements dropped

		if(S_OK!=VariantClear(pNewToolIds))
			return E_FAIL;
		pNewToolIds->vt = VT_ARRAY | VT_VARIANT;
		pNewToolIds->parray = SafeArrayCreate(VT_VARIANT, 1, &rgsaBound);
		...
		while(!pItr->done())
		{
			...
			if(pItr->getEntity(pDropEntity, AcDb::kForRead)==Acad::eOk)
				// Demonstrate how to create a tool for every <child> in the drop...
			{
				CComObject<TT> tool;
				tool.New();
				if(S_OK==tool.dropCallback(pDropEntity)) // Allow the Child class to handle how the tools are created when content is dropped.	
				{
					if(Acad::eOk!=pDropEntity->close())
					{
						SafeArrayDestroyData(pNewToolIds->parray);
						delete pItr;
						return E_FAIL;
					}

					// Create the tool...
					AcTcTool *pAcTcTool=tool.CreateToolATC(pParent,szToolName,szToolImage);
					if(!pAcTcTool)
					{
						SafeArrayDestroyData(pNewToolIds->parray);
						delete pItr;
					...
					GUID toolGuid;
					pAcTcTool->GetID(&toolGuid);
					
					// Now, add it as a VARIANT BSTR to the SafeArray pointer passed in as pNewToolIds
					VARIANT varGuid;
					varGuid.vt = VT_BSTR;
					varGuid.bstrVal=StringFromGuid(toolGuid);
					if(S_OK!=SafeArrayPutElement(pNewToolIds->parray, &toolIndex,&varGuid))
					{
						SafeArrayDestroyData(pNewToolIds->parray);
						delete pItr;
						delete pAcTcTool;
		...
		// Now, resize the array for the number of tools actually dropped.
		rgsaBound.cElements = toolIndex;
		SafeArrayRedim(pNewToolIds->parray,&rgsaBound);

		data.Detach();
		return S_OK;
	}

	STDMETHOD (GetDragDropContextMenuItem) (IUnknown * pIUnknown,BSTR * pbstrText,DWORD * pFlag)
	{
		return E_NOTIMPL;
	}
	...
	STDMETHOD (GetUnspecifiedString)(VARIANT,BSTR *str)
	{
		...
		_bstr_t unspecifiedString(_T("*Unspecified*"));
	...
	HRESULT GetPropertyTypeInfo()
	{
		...
		if (!SUCCEEDED(QueryInterface(*piid,(void**)&m_pITool)))
			return E_FAIL;
		// Find out if we have Type Info (we should have)
		unsigned int count = 0;
		if (!SUCCEEDED(m_pITool->GetTypeInfoCount(&count)))
			return E_FAIL;
		...
		if (!SUCCEEDED(m_pITool->GetTypeInfo(0, NULL, &pTypeInfo)))
			return E_FAIL;
		...
		for(int i = 0; i < numFuncs; i++)
		{
			...
			if(memberID < 1000) // Assumes the IDs below 1000 are the child class' methods
			{
					...
					_tcscpy(mapEntry.szDesc,szName);
					mapEntry.dispId=memberID;
					m_ToolMap.append(mapEntry);
					
					// Determine the Color and Layer DISPIDs.
					if(!_tcscmp(szName,"Color"))
						dispColor=memberID;
					if(!_tcscmp(szName,"Layer"))
						dispLayer=memberID;
					if(!_tcscmp(szName,"Flyout"))
						dispFlyout=memberID;
					delete pBstrNames;
			}
		}
	...
	{
		IAcadToolProperties2 *pToolProp2=GetThisToolsProperties();
		if(!pToolProp2)
			return E_FAIL;
		...
		//
		GUID id;
		pToolProp2->get_ShapePackageId(&id);
		...
		VARIANT varShapes;
		HRESULT hr = pToolProp2->get_ActiveShapeIds(&varShapes);
		...
		//
		GUID currentShapeId;
		hr = pToolProp2->get_CurrentShapeId(&currentShapeId);
		...
		int nCount = activeShapes.GetSize();
		...
		hr=pToolProp2->put_ActiveShapeIds(&varShapes);
    ...
    STDMETHOD(SetFlyoutTools) (/* [in] */ VARIANT* pFlyoutToolIds)
    {
		...
		for(long i=lLower; i<=lUpper; i++) {
			...
			_tcscpy(m_FlyoutTools,CW2A(val.bstrVal));
		}
		return S_OK;
    }
	...
	STDMETHOD(ShowProperty)(DISPID dispID,BOOL * pShow)
	{
		...
		*pShow=TRUE;
		...
		pToolProp2->get_IsFlyoutEnabled(&vbFlyout);
		if(vbFlyout)
		{
			PER_FLYOUT_ENTRY *pFlyoutMap=TT::GetFlyoutMap();	
...
BOOL GuidFromString(LPTSTR pszGuid, GUID& id)
{
	...
	TCHAR tsFormatGuid[64]=_T("{");
	if(_tcslen(pszGuid) > 0 && pszGuid[0] !=_T('{'))
		_tcscat(tsFormatGuid,pszGuid);
	if(_tcslen(pszGuid) > 0 && pszGuid[_tcslen(tsFormatGuid)-1] !=_T('}'))
		_tcscat(tsFormatGuid,_T("}"));
...
CComBSTR StringFromGuid(const GUID& id)
{
	...
	STDMETHOD (put_Color)(VARIANT pVar)
	{
		...
		m_Color.setNames(CW2A(colorName),CW2A(bookName));
		return S_OK;
	}
	...
	{
		CComBSTR bstr(_T("<Choose Commands>"));
		bstr.CopyTo(pVar);
		return S_OK;
	}
		...
		try {
			...
			*ppChild = NULL;
			...
			for (long i=0; i<nCount; i++) {
				...
				if(_tcsncicmp(szName,pszNodeName,_tcslen(szName)) == 0) {
								...
								int index)
	{
		...
		if(dispColor==index)
		{
			...
			if(GetChildNode(pNode, "TrueColor",&pColorNode))
			{
				// \ TrueColor
				if(GetChildNode(pColorNode,"ColorValue",&pColorBookNode))
				{
					// \TrueColor\colorValue
					GetAttributeValue(pColorBookNode, "Unspecified", (long&)m_bColorUnspecified);
					long color;
					GetAttributeValue(pColorBookNode, "Color", color);
					m_Color.setColor(color);
				}
				TCHAR sColorName[512]={""},sColorBook[512]={""};
				if(GetChildNode(pColorNode,"ColorName",&pColorBookNode))
					// \TrueColor\ColorName
					GetAttributeValue(pColorBookNode, "Color", sColorName);
				if(GetChildNode(pColorNode,"ColorBook",&pColorBookNode))
					// \TrueColor\ColorBook
					GetAttributeValue(pColorBookNode,"Color",sColorBook);
				...
				const char* pNullName = NULL;
				m_Color.setNames( _tcslen(sColorName) ? sColorName : pNullName, 
					_tcslen(sColorBook) ? sColorBook : pNullName);
			}
			return TRUE;
		}
		
		if(dispLayer == index) 
			GetAttributeValue(pNode, "Unspecified", (long&)m_bLayerUnspecified);
		
		GetAttributeValue(pNode,"PropertyValue",vValue); // Get the value from the XML file through the VARIANT.
								...
								int index)
	{
		...
		if(dispColor==index)
		{
			...
			MSXML::IXMLDOMNodePtr pColorNode,pColorBookNode;
			if(!GetChildNode(pNode,"TrueColor",&pColorNode))
				AddChildNode(pNode, "TrueColor", MSXML::NODE_ELEMENT, &pColorNode);
			if (pColorNode == NULL)
				return FALSE;

			// \TrueColor \ColorValue
			if(!GetChildNode(pColorNode,"ColorValue",&pColorBookNode))
				AddChildNode(pColorNode, "ColorValue", MSXML::NODE_ELEMENT, &pColorBookNode);
			SetAttributeValue(pColorBookNode, "Unspecified", (long&)m_bColorUnspecified);
			long color=m_Color.color();
			SetAttributeValue(pColorBookNode, "Color",color);

			LPTSTR pColorName = (LPTSTR)m_Color.colorName();
			if (pColorName) 
			{
				// \TrueColor\ColorName
				if(!GetChildNode(pColorNode,"ColorName",&pColorBookNode))			
					AddChildNode(pColorNode, "ColorName", MSXML::NODE_ELEMENT, &pColorBookNode);
				SetAttributeValue(pColorBookNode,"Color",pColorName);
			}

			LPTSTR pBookName = (LPTSTR)m_Color.bookName();
			if(pBookName) 
			{
				// \TrueColor\ColorBook
				if(!GetChildNode(pColorNode,"ColorBook",&pColorBookNode))
					AddChildNode(pColorNode, "ColorBook",MSXML::NODE_ELEMENT, &pColorBookNode);
				SetAttributeValue(pColorBookNode,"Color",pBookName);
			}
			return TRUE;
		}
		...
		if(dispLayer==index)
			SetAttributeValue(pNode, "Unspecified", (long)m_bLayerUnspecified);
		
		return SetAttributeValue(pNode,"PropertyValue",vValue); // Set the value in the XML file through the VARIANT.
	}
	...
	AcTcPalette *CreatePaletteATC(AcTcCatalog *pCatalog,LPTSTR szPaletteName)
	{
		GUID guid;
	...
	AcTcTool *CreateToolATC(AcTcPackage *pPalette, LPCTSTR szToolNameOverride=NULL, LPCTSTR szToolImageOverride=NULL)
	{
		GUID guid;
		BOOL bResult;
		...
		bResult=AcTcGetManager()->SaveCatalogs();	
		
		return pTool;
	}

	AcTcTool *CreateFlyoutToolATC(AcTcPackage *pPalette, AcTcPackage *pShapePackage, LPCTSTR szToolNameOverride=NULL)
	{
		GUID toolGUID;
		BOOL bResult;
		...
		if( pShapePackage->GetCustomData(&pCustomDataUnk) )
		{
			...
			if(!GetChildNode(pCustomDataNode,_T("ActiveShapes"),&pActiveShape))
				AddChildNode(pCustomDataNode,_T("ActiveShapes"),MSXML::NODE_ELEMENT,&pActiveShape);
			for(int i=0;i<pShapePackage->GetChildCount();i++)
			{
				GUID shapeGUID;
				pShapePackage->GetChild(i)->GetID(&shapeGUID);
				CComVariant varGUID(shapeGUID);
				AddChildNode(pActiveShape,_T("Shape"),MSXML::NODE_ELEMENT,&pChild);
				SetAttributeValue(pChild,_T("idValue"),varGUID);
			}
		}
		*/
		
		bResult=AcTcGetManager()->SaveCatalogs();
		bResult=AcTcGetManager()->SaveCatalogs(AcTc::kShapeCatalog);
		return pTool;
	}

	AcTcTool *CreateCommandToolATC(AcTcPackage *pPalette,LPTSTR szToolName,LPCTSTR szBitmapName, LPTSTR macroString)
	{
		...
		if(pTool->GetToolData(&pToolDataUnk))
		{
			...
			if(TRUE == AddChildNode(pToolData, _T("Macro"), MSXML::NODE_ELEMENT, &pNode))
				if(TRUE == AddChildNode(pNode, _T(""), MSXML::NODE_CDATA_SECTION, &pNodeText))
		...
		bResult=AcTcGetManager()->SaveCatalogs();// AutoCAD will create the correct ATC files within the roamable profile folder.
		bResult=AcTcGetManager()->SaveCatalogs(AcTc::kShapeCatalog);
		return pTool;
	}

	AcTcCatalog *CreateStockToolATC(LPTSTR szCatalogName)
	{
		...
		pStockToolCatalog->AddChild(pStockTool);
		if(!AcTcGetManager()->AddStockToolCatalog(pStockToolCatalog))
			return NULL;
		...
		if(!AcTcGetManager()->AddCatalog(pCatalog))
		{
			delete pCatalog;
			return NULL;
		}
		...
		bResult=AcTcGetManager()->SaveCatalogs(AcTc::kStockToolCatalog);
		return pCatalog;
	}

	AcTcPackage *CreateShapeCatalogATC(LPTSTR szShapeName)
	{
		BOOL bResult;
		...
		if (NULL==pPackage)
		{
			...
			pShapesCatalog->AddChild(pPackage);
		}
		bResult=AcTcGetManager()->SaveCatalogs(AcTc::kShapeCatalog);
		return pPackage;
	}
	...
	IAcadToolProperties2* GetThisToolsProperties()
	{
		...
		CComQIPtr<IAcadToolProperties2> pToolProp2(pUnkToolProp);
		if(!pToolProp2)
			return NULL;
							...
							LPCTSTR szPackageName)
	{
		...
		for(int i=0;i<pCatalog->GetChildCount();i++)
		{
			...
			if(pPackage)
			{
				TCHAR szPackageNameEnum[128];
				pPackage->GetName(szPackageNameEnum,128);
				if(!_tcscmp(szPackageName,szPackageNameEnum))
					return pPackage;
			}
		}	
		return NULL;
