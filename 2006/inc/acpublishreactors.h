...
public:
    ...
    /// </param>
    ///
    AcNameValuePair(const char *name, const char *value)
    {
        if (NULL != name) {
...
private:
    // these are char and not TCHAR since we are
    // storing them in an INI file
    char * m_pName;
    char * m_pValue;    
};

typedef std::vector<AcNameValuePair> AcNameValuePairVec;
...
public:
    ...
    virtual const AcNameValuePairVec 
        GetPrivateData(const char * sectionName) = 0;
    ...
    /// </returns>
    virtual bool WritePrivateSection(const char * sectionName, 
...
public:
    ...
    /// </returns>
    virtual const TCHAR * GetUniqueId() = 0;
...
public:
    ...
    /// </returns>
    virtual const char* dwfFileName() = 0;
    ...
    /// </returns>
    virtual const char* dwfPassword() = 0;
};
...
public:
    ...
    /// page
    /// </returns>
    virtual const char* tempDwfFileName() = 0;
...
/// 
/// a)  First check to see if the AcPublish.arx is loaded or not. This you can 
/// do by doing the following:
/// 
/// if(!acrxServiceIsRegistered(/*MSG0*/"AdskPublish")) {
///     acrxLoadModule(/*MSG0*/"AcPublish.arx", false, false);
/// }
/// 
/// b)  Once the AcPublish.arx module is loaded, do the following:
/// 
...
/// reactor notifiers) responsibility to make sure AcPublish.arx is loaded 
...
typedef (__cdecl * ACGLOBADDPUBLISHREACTOR)(AcPublishReactor * pReactor);
typedef (__cdecl * ACGLOBREMOVEPUBLISHREACTOR)(AcPublishReactor * pReactor);
