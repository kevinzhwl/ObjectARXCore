// mAlignmentDirections.append(alignmentVectors);
//
...
class AcDbAppEntityGripData : public AcHeapOperators
{
public:
    AcDbAppEntityGripData(const unsigned long id);
    AcDbAppEntityGripData(const AcDbAppEntityGripData& src);
    ~AcDbAppEntityGripData();

    AcDbAppEntityGripData& operator = (const AcDbAppEntityGripData& src);

    bool alignmentGripType() const;
    bool rotationGripType() const;

protected:
    friend class SBAppData;
    AcDbAppEntityGripData();
    void        *   mpImpObj;
};
