...
//	PROMPT_ENTRY(
//		_T("DESCRIPTION"),		- String describing the entry
...
class ATL_NO_VTABLE AsdkPromptBase :
	...
	public CComCoClass<AsdkPromptBase, NULL>,
...
protected:
	...
	void setPropertyDefault (int i) {
		...
		switch ( pPromptMap [i].m_paramType ) {
			...
			case VT_ARRAY | VT_R8: {
				...
				_tcscpy (szDefault, szDefaultValue) ;
				AcGePoint3d defPt ;
				TCHAR *token =_tcstok (szDefault, seps) ;
				...
				if ( updateCallback (i) == false ) {
					switch ( pPromptMap [i].m_promptType ) {
						...
						case RTSTR:
							...
							break ;
						default:
							acutPrintf ("Type %s not handled by Jig mechanism!\n", pPromptMap [i].m_szDesc) ;
							break ;
					}
					PutProperty (i) ;
					pPromptMap [i].m_bGotThisProperty =false ;
	...
	virtual bool defaultValueCallback (int index, LPTSTR szDefaultVal) { return (false) ; }

	//- Utility
	int LoadString (UINT uID, LPTSTR szString, int nBufferMax =255) {
		int ret =0 ;
		*szString =_T('\0') ;
		if ( uID != 0 && uID != (UINT)-1 )
	...
	LPCTSTR BuildPromptString (int index) {
		PROMPT_MAP_ENTRY *pPromptMap =GetPromptMap () ;
		...
		*szDefaultVal =_T('\0') ;
		...
		if ( *szDefaultVal != _T('\0') ) {
			_tcscat (szPrompt, _T(" <")) ;
			_tcscat (szPrompt, szDefaultVal) ;
			_tcscat (szPrompt, _T(">")) ;
		}
		_tcscat (szPrompt, _T(": ")) ;
		return (szPrompt) ;
			...
			while ( pPromptMap [i].m_szDesc ) { 
				if ( !pPromptMap [i].m_bJigThisProperty && !pPromptMap [i].m_bGotThisProperty ) {
					...
					if ( retval == RTINPUTTRUNCATED ) { //- return RTINPUTTRUNCATED to use the defaults
						...
						LPCTSTR szPrompt =BuildPromptString (i) ;
						switch ( pPromptMap [i].m_promptType ) {
							case RTPOINT: {
								ads_point point ;
							...
							case RTSTR: {
								...
								*szBuffer =_T('\0') ;
								...
								if ( retval == RTNORM ) {
									if ( _tcslen (szBuffer) ) //- No zero length strings...
...
#define BEGIN_PROMPT_MAP(CLASSNAME) \
	...
	CLASSNAME () { \
		...
		if ( FAILED(hr =m_pWrapperObject.CoCreateInstance (m_CLSID_ObjectWrapper)) ) { \
			acutPrintf ("Unable to Create Wrapper Class\n") ; \
			throw hr ; \
		} \
		...
		if ( FAILED(hr =GetPropertyTypeInfo ()) ) { \
			acutPrintf ("Unable to Obtain Correct Type Information from the Wrapper Class\n") ; \
			throw hr ; \
		} \
