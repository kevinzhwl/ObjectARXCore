...
public:
    ...
    {
        if (NULL != name) {
            m_pName = new ACHAR[_tcslen(name) + 1];
            _tcscpy(m_pName, name);
        } else
            m_pName = NULL;
        
        if (NULL != value) {
            m_pValue = new ACHAR[_tcslen(value) + 1];
            _tcscpy(m_pValue, value);
        } else
            m_pValue = NULL;
    }
    ...
    /// </returns>
    const ACHAR * name() {return m_pName;};
    ...
    /// </returns>
    const ACHAR * value() {return m_pValue;};
