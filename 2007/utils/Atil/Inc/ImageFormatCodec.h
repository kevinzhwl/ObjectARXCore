...
namespace Atil { class FileHandleInterface; }
#endif
...
public:
    ...
    /// <summary>
    /// This query returns the appropriate value from the <c>FormatDecodability</c> 
    /// <see cref="FormatDecodability"/>
    /// </summary>
    ///
    ...
    /// <summary>
    /// This query returns the appropriate value for the format from the 
    /// <c>isFormatFeature</c><see cref="isFormatFeature"/> enum.
    /// </summary>
    ///
    ...
    /// </summary>
    ///
    /// <param name="propId">A definition from the <c>FormatCodecPropertyInterface</c>
    /// that indicates the property that is of interest.
    /// </param>
    ///
    /// <param name="pCustomProptertyName">A <c>String buffer pointer</c> (or NULL) that
    /// holds the "name" of a property. This string match system can be used
    /// to query for non-ATIL defined properties.
    /// </param>
    ///
    /// <returns>This will return true if the property is supported by the codec.
    /// </returns>
    ///
    virtual bool isSupportedProperty (
        FormatCodecPropertyInterface::PropertyIdentifier propId,
        StringBuffer* pCustomPropertyName = NULL) const = 0;
    ...
    /// identify an image file encoding. Each register <c>ImageFormatCodec</c> instance
    ...
    virtual FormatDecodability isDecodableFile (const FileSpecifier& fileSpecifier,
        FindFileInterface * ff = NULL) const = 0;
