...
public:
    ...
    // Methods that allow database-resident objects to keep a GUID that is changed 
    // every time the contents of the object changes. This way it is possible to check 
    // whether the contents of the object changed. The GUID generation can be enabled 
    // or disabled. If enabled, a new GUID is automatically generated when the contents
    // of the object is changed. The client code may also generate a new GUID by 
    // calling generateDifferentObjectChangeGuid(). 
    //
    // The GUID generation is not enabled by default and needs to be explicitly enabled
    // before it can be used. It is applicable to database-resident entities only, because 
    // it might be an overkill to do it for all transient non-database resident entities.
    //
    ACDB_PORT bool                isObjectChangeGuidEnabled() const;
    ACDB_PORT const struct _GUID& getObjectChangeGuid() const;
    ACDB_PORT void                enableObjectChangeGuid(bool yesNo);
    ACDB_PORT void                generateDifferentObjectChangeGuid(); 
};
