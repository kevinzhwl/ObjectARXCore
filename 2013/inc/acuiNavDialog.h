...
class ACUI_PORT CAcUiNavDialog : public CNavDialog {
    ...
    public:
        ...
        BOOL GetSaveToCloud();                  // Does the user want to save this drawing to the cloud
        int  GetCloudLocation();                // Get the location of the cloud places item
        BOOL GetCloudFolder(CString & folder);  // Get the sysvar ONLINEFOLDER
        BOOL ResetToCloudLocation();
        void SetCloudUIState();                 // Manage the display of custom controls
        void SetCloudControlsVisible(BOOL show);   // Display cloud controls when cloud selected
...
#pragma warning(pop)
    ...
    //{{AFX_MSG(CAcUiNavDialog)
        ...
        afx_msg void OnPlacesChanged();         // Places list box changed
    ...
    private:
        ...
        BOOL m_bSaveToCloud;                   // Save the local file to the cloud.
        BOOL m_bCloudControlsVisible;          //Override cloud controls
        ...
        CAdUiBitmapStatic * m_cloudDocsBitmapButton;
        ...
        void GroupBoxTextDisplay(BOOL forCloud);
        void GetCloudMode();
        bool m_bUseRememberFolders;
        void SetWebUserRegValues();
        void GetWebUserRegValues();
        BOOL ResetPlacesOnCloudLoginFailure();
        void HideCloudControls();
