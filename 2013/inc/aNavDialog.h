...
public:
    ...
    // Get/set the open/save dialog so that it will select and access the cloud folder.
    BOOL    GetForceCloudInitialFolder();
    void    SetForceCloudInitialFolder(BOOL bForce);
    ...
    // Get/set whether the cloud extension is present or not in the folder
    // combo and places list
    BOOL    GetNoCloud();
    void    SetNoCloud(BOOL noCloud);
    ...
    // Called to acquire the rx value sysvar of the cloud
    void SetCloudFolder(LPCTSTR strFolder);
