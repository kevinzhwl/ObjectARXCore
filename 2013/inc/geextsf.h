...
public:
   ...
   // Creates an (unbounded) AcGeSurface from the surface geometry of a given ASM face.
   // If the face surface can be represented by a native AcDbSurface, it creates a 
   // native AcGeSurface, otherwise it creates an AcGeExternalSurface
   //
   static void createFromAsmFaceSurface(const void* pAsmFaceAsVoid, AcGeSurface*& pGeSurface);
};
